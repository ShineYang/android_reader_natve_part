...
package com.shawnyang.japanese_reader_android.data
...
class BooksDatabaseOpenHelper(ctx: Context) : ManagedSQLiteOpenHelper(ctx, "books_database", null, DATABASE_VERSION) {
    ...
    private fun upgradeVersion2(db: SQLiteDatabase, callback: () -> Unit) {
        ...
        val cursor = db.query(
            BOOKSTable.NAME,
            BOOKSTable.RESULT_COLUMNS, null, null, null, null, null, null)
        if (cursor != null) {
...
class BOOKS(private var database: BooksDatabaseOpenHelper) {
    ...
    private fun has(book: Book): List<Book> {
        return database.use {
            select(
                ...
                BOOKSTable.HREF,
                BOOKSTable.ID,
                BOOKSTable.IDENTIFIER,
                BOOKSTable.COVER,
                BOOKSTable.EXTENSION,
                BOOKSTable.CREATION,
                BOOKSTable.PROGRESSION
            )
    ...
    fun deleteById(id: String): Int {
        return database.use {
            return@use delete(BOOKSTable.NAME, "id = {id}", "id" to id)
        }
    }

    fun queryById(id: String): Book {
        return database.use {
            return@use select(
                BOOKSTable.NAME,
                BOOKSTable.TITLE,
                ...
                BOOKSTable.EXTENSION,
                BOOKSTable.CREATION,
                BOOKSTable.PROGRESSION).whereArgs("id = {id}", "id" to id)
                .exec {
                    parseSingle(MyRowParser())
                }
        }
    }
